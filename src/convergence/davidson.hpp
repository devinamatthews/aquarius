...
class Davidson
{
    ...
    protected:
        vector<vector<tensor::Tensor<>>> old_c; // hold all R[k][i] where i is over nvec and k is over maxextrap
        vector<vector<tensor::Tensor<>>> old_hc; // hold all H*R[i] = Z[i] at every iteration aka Z[k][i]
        vector<tensor::ConstTensor<>> guess;
        marray<tensor::Scalar,3> guess_overlap;
        marray<tensor::Scalar,4> s, e; // e will hold chc[k]
        vector<tensor::Scalar> c;
        int nvec, maxextrap, nextrap; // number of energies, number of iterations
        vector<int> mode;
        vector<tensor::Scalar> target;
        vector<tensor::Scalar> previous;
        vector<int> root;
        vector<tensor::Scalar> l;
        marray<tensor::Scalar,3> vr;
        ...
        void init(const input::Config& config)
        {
            ...
            root.resize(nvec);
            l.resize(nvec*maxextrap, tensor::Scalar(tensor::Field::DCOMPLEX));
    ...
    public:
        ...
        Davidson(const input::Config& config, const tensor::Scalar& target)
        : nvec(1), mode{CLOSEST_ENERGY}, target{target}, previous(1)
        { init(config); }

        Davidson(const input::Config& config, const vector<tensor::Scalar>& target)
        : nvec(target.size()), mode(target.size(),CLOSEST_ENERGY),
          target(target), previous(target.size())
        { init(config); }

        Davidson(const input::Config& config, const tensor::ConstTensor<>& guess)
        : guess{guess}, nvec(1), mode{GUESS_OVERLAP}, target(1), previous(1)
        { init(config); }

        Davidson(const input::Config& config, const vector<tensor::ConstTensor<>>& guess)
        : guess(guess), nvec(guess.size()), mode(guess.size(),GUESS_OVERLAP),
          target(guess.size()), previous(guess.size())
        { init(config); }

        tensor::Scalar extrapolate(tensor::Tensor<>&& c, tensor::Tensor<>&& hc, const op::Denominator& D)
        {
            assert(nvec == 1);
            return extrapolate({c}, {hc}, D)[0];
        }

        vector<tensor::Scalar> extrapolate(vector<tensor::Tensor<>>&& c, vector<tensor::Tensor<>>&& hc, const op::Denominator& D)
        {
            ...
            {
                tensor::Scalar norm = sqrt(abs(scalar(conj(c[i])*c[i])));
            ...
            for (int i = 0;i < nvec;i++)
            {
                ...
                if (!guess.empty())
                {
                    ...
                    {
                        guess_overlap[j][i][nextrap] = abs(scalar(conj(c[i])*guess[j]));
                    }
                }
