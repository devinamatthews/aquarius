 ...
 *
 * The general form for an outer product is \f$ab\ldots \times cd\ldots \rightarrow ab\ldots cd\ldots\f$
...
int tensor_outer_prod_dense_(const double alpha, const double* restrict A, const int ndim_A, const int* restrict len_A, const int* restrict lda, const int* restrict idx_A,
                             ...
                             const double beta,        double* restrict C, const int ndim_C, const int* restrict len_C, const int* restrict ldc, const int* restrict idx_C)
{
    ...
    /*
     * sort the indices into two groups: A and C, and B and C
     */
    for (i = 0;i < ndim_A;i++)
    {
#ifdef VALIDATE_INPUTS
        ...
        for (j = 0;j < ndim_B;j++)
        {
            if (idx_A[i] == idx_B[j]) return TENSOR_INDEX_MISMATCH;
        }
    ...
    for (i = 0;i < ndim_B;i++)
    {
        ...
        for (j = 0;j < ndim_A;j++)
        {
            if (idx_B[i] == idx_A[j]) return TENSOR_INDEX_MISMATCH;
        }
    ...
    for (i = 0;i < ndim_C;i++)
    {
        ...
        for (j = 0;j < ndim_B;j++)
        {
            if (idx_C[i] == idx_B[j]) found = true;
        }

        for (j = 0;j < ndim_A;j++)
        {
            if (idx_C[i] == idx_A[j]) found = true;
        }
