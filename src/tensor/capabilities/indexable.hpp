#ifndef _AQUARIUS_TENSOR_INDEXABLE_TENSOR_HPP_
#define _AQUARIUS_TENSOR_INDEXABLE_TENSOR_HPP_
...
template <> class TensorInitializer<INDEXABLE> : public Destructible
{
    public:
        int ndim;
...
TENSOR_DEFINITION(INDEXABLE)
{
    public:
        ...
        {
            this->template as<INDEXABLE>().mult(alpha, conja, A, detail::implicit(A.as<INDEXABLE>().getDimension()),
                                              conjb, B, detail::implicit(B.as<INDEXABLE>().getDimension()),
                                        beta,           detail::implicit(                  getDimension()));
        }

        void sum(const Scalar& alpha, bool conja, const TensorImplementation<>& A, const Scalar& beta)
        {
            this->template as<INDEXABLE>().sum(alpha, conja, A, detail::implicit(A.as<INDEXABLE>().getDimension()),
                                       beta,           detail::implicit(                  getDimension()));
        }

        void scale(const Scalar& alpha)
        {
            this->template as<INDEXABLE>().scale(alpha, detail::implicit(getDimension()));
        }

        Scalar dot(bool conja, const TensorImplementation<>& A, bool conjb) const
        {
            return this->template as<INDEXABLE>().dot(conja, A, detail::implicit(A.as<INDEXABLE>().getDimension()),
...
inline Scalar scalar(const tensor::IndexedTensorMult& itm)
{
    return itm.factor*itm.B.as<INDEXABLE>().dot(itm.conja, itm.A, itm.idxa,
                                                itm.conjb,        itm.idxb);
}
