...
template <typename U>
CCSDT_Q_Density<U>::CCSDT_Q_Density(const string& name, Config& config)
: Task(name, config)
{
    vector<Requirement> reqs;
    reqs.push_back(Requirement("moints", "H"));
    reqs.push_back(Requirement("ccsdt.T", "T"));
    reqs.push_back(Requirement("ccsdt(q).L", "L"));
    reqs.push_back(Requirement("ccsdt(q).G", "G"));
    this->addProduct(Product("ccsdt(q).D", "D", reqs));
}

template <typename U>
bool CCSDT_Q_Density<U>::run(TaskDAG& dag, const Arena& arena)
{
    ...
    auto& G = this->template get<TwoElectronOperator<U>>("G");
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CCSDT_Q_Density);
REGISTER_TASK(aquarius::cc::CCSDT_Q_Density<double>,"ccsdt(q)_density","");
