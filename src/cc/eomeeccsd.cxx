...
bool EOMEECCSD<U>::run(TaskDAG& dag, const Arena& arena)
{
    ...
    auto& Es = this->put   ("energy", new vector<vector<U>>());
    auto& Rs = this->put   ("R", new vector<unique_vector<ExcitationOperator<U,2>>>());
    auto& Cs = this->puttmp("C", new unique_vector<ExcitationOperator<U,2>>());
        ...
        for (int spin : {0,1})
        {
            ...
            else
            {
                ...
                for (auto& root : roots)
                {
                    ...
                    ExcitationOperator<U,2>& R = Cs.back();
                    R(0) = 0;
                    ...
                    Es[irrep].push_back(this->energy());
                    Rs[irrep].emplace_back(R);
...
template <typename U>
LeftEOMEECCSD<U>::LeftEOMEECCSD(const string& name, Config& config)
: Iterative<U>(name, config),
  davidson_config(config.get("davidson")),
  multiroot(config.get<bool>("multiroot"))
{
...
template <typename U>
bool LeftEOMEECCSD<U>::run(TaskDAG& dag, const Arena& arena)
{
    auto& H = this->template get<STTwoElectronOperator<U>>("Hbar");
    ...
    auto& Es = this->template get<vector<vector<U>>>("energy");
    auto& Rs = this->template get<vector<unique_vector<ExcitationOperator<U,2>>>>("R");

    auto& Ls = this->put   ("L", new vector<unique_vector<DeexcitationOperator<U,2>>>());
    ...
    for (int irrep = 0;irrep < nirrep;irrep++)
    {
        ...
        else
        {
            ...
            for (int root = 0;root < Rs[irrep].size();root++)
            {
                auto& R = Rs[irrep][root];
                U E = Es[irrep][root];
...
template <typename U>
void LeftEOMEECCSD<U>::iterate(const Arena& arena)
{
    const auto& H = this->template get<STTwoElectronOperator<U>>("Hbar");
...
REGISTER_TASK(aquarius::cc::LeftEOMEECCSD<double>, "lheomeeccsd",spec2);
