#include "../../frameworks/cc/ccsdtq_3.hpp"
...
template <typename U>
CCSDTQ_3<U>::CCSDTQ_3(const string& name, Config& config)
: Iterative<U>(name, config), diis(config.get("diis"))
{
    ...
    this->addProduct(Product("ccsdtq-3.T", "T", reqs));
    this->addProduct(Product("ccsdtq-3.Hbar", "Hbar", reqs));
}

template <typename U>
bool CCSDTQ_3<U>::run(task::TaskDAG& dag, const Arena& arena)
{
...
template <typename U>
void CCSDTQ_3<U>::iterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CCSDTQ_3);
REGISTER_TASK(aquarius::cc::CCSDTQ_3<double>,"ccsdtq-3",spec);
