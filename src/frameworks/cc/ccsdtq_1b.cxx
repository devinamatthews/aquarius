...
template <typename U>
CCSDTQ_1b<U>::CCSDTQ_1b(const string& name, Config& config)
: Iterative<U>(name, config), diis(config.get("diis"))
{
    ...
    this->addProduct(Product("ccsdtq-1b.T", "T", reqs));
    this->addProduct(Product("ccsdtq-1b.Hbar", "Hbar", reqs));
}

template <typename U>
bool CCSDTQ_1b<U>::run(task::TaskDAG& dag, const Arena& arena)
{
...
template <typename U>
void CCSDTQ_1b<U>::iterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CCSDTQ_1b);
REGISTER_TASK(aquarius::cc::CCSDTQ_1b<double>,"ccsdtq-1b",spec);
