...
template <typename U>
CCSDTQ_1b_Density<U>::CCSDTQ_1b_Density(const string& name, Config& config)
: Task(name, config)
{
    ...
    reqs.push_back(Requirement("ccsdtq-1b.L", "L"));
    this->addProduct(Product("ccsdtq-1b.D", "D", reqs));
}

template <typename U>
bool CCSDTQ_1b_Density<U>::run(TaskDAG& dag, const Arena& arena)
{
    ...
    GAIJK["aijk"]  =  (1.0/ 12.0)*L(3)["inoefg"]*  T4["aefgjkno"];

    GABCI["abci"]  = -(1.0/ 12.0)*L(3)["mnocfg"]*  T4["abfgmino"];
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CCSDTQ_1b_Density);
REGISTER_TASK(aquarius::cc::CCSDTQ_1b_Density<double>,"ccsdtq-1b_density","");
