#include "../../frameworks/cc/lambdaccsdtq_1b.hpp"
...
template <typename U>
LambdaCCSDTQ_1b<U>::LambdaCCSDTQ_1b(const string& name, Config& config)
: Iterative<U>(name, config), diis(config.get("diis"))
{
    vector<Requirement> reqs;
    reqs.push_back(Requirement("moints", "H"));
    reqs.push_back(Requirement("ccsdtq-1b.Hbar", "Hbar"));
    reqs.push_back(Requirement("ccsdtq-1b.T", "T"));
    this->addProduct(Product("double", "energy", reqs));
    this->addProduct(Product("double", "convergence", reqs));
    this->addProduct(Product("ccsdtq-1b.L", "L", reqs));
}

template <typename U>
bool LambdaCCSDTQ_1b<U>::run(TaskDAG& dag, const Arena& arena)
{
...
template <typename U>
void LambdaCCSDTQ_1b<U>::iterate(const Arena& arena)
{
    const auto& H    = this->template get<  TwoElectronOperator<U>>("H");
    const auto& Hbar = this->template get<STTwoElectronOperator<U>>("Hbar");
    ...
    /***************************************************************************
     *
     * Intermediates for Lambda-CCSDTQ-1b
     */
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::LambdaCCSDTQ_1b);
REGISTER_TASK(aquarius::cc::LambdaCCSDTQ_1b<double>,"lambdaccsdtq-1b",spec);
