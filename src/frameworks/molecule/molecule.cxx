...
void Molecule::initSymmetry(const Config& config, vector<AtomCartSpec>& cartpos)
{
    ...
    else if (2*std::abs(A[0]-A[2])/(A[0]+A[2]) < 1e-8)
    {
        /*
    ...
    else if (2*std::abs(A[0]-A[1])/(A[0]+A[1]) < 1e-8 ||
             2*std::abs(A[1]-A[2])/(A[1]+A[2]) < 1e-8)
    {
        /*
         * Symmetric rotors: Cn, Cnv, Cnh, Dn, Dnh (all n>2), S2n, Dnd (both n>1)
         */
        vec3 axis = (2*std::abs(A[0]-A[1])/(A[0]+A[1]) < 1e-8 ? z : x);
