...
#if 0
template <typename T>
typename AOMOIntegrals<T>::pqrs_integrals
AOMOIntegrals<T>::pqrs_integrals::transform(Index index, const vector<int>& nc, const vector<vector<T> >& C, bool pleq)
{
    //TODO
    pqrs_integrals out;
...
template <typename T>
void AOMOIntegrals<T>::pqrs_integrals::transcribe(SymmetryBlockedTensor<T>& symtensor, bool assymij, bool assymkl, Side swap)
{
    PROFILE_FUNCTION

    if (assymij) assert(np == nr);
    if (assymkl) assert(nq == ns);
        ...
        for (int irrr = 0;irrr < n;irrr++)
        {
            ...
            for (int irrq = 0;irrb < n;irrb++)
            {
                ...
                for (int irra = 0;irra < n;irra++)
                {
                    ...
                    symirrs[2] = irrb;
                    symirrs[3] = irrs;
                    if (swap == PQ || (assymij && irra > irrr)) swap(symirrs[0], symirrs[1]);
                    if (swap == RS || (assymkl && irrb > irrs)) swap(symirrs[2], symirrs[3]);
...
template <typename T>
void AOMOIntegrals<T>::pqrs_integrals::transpose()
{
    for (auto& idx : idxs)
    {
        swap(idx.i, idx.k);
